...
public class CharacterManager {
  ...
  public static CharacterBase getCharacterByName(String characterName, String language) {
    return characters.values().stream()
        .filter(
            character ->
                character
                    .getCharacterInformation(language)
                    .getName()
   ...
   */
  public static String formatCharacterDialog(
      CharacterBase.Information characterInformation, CharacterBase.Dialog dialog) {
    String text =
        "\uD83D\uDDE3\uFE0F  **" + characterInformation.getName() + "** - " + dialog.getText();
   ...
   */
  private static CharacterBase.Information loadCharacterByLanguage(
      String id, JsonObject characterObject, String language) {
    String characterName =
        characterObject.get("name").getAsJsonObject().get(language).getAsString();
    String characterDescription =
        characterObject.get("description").getAsJsonObject().get(language).getAsString();
   ...
   */
  private static HashMap<String, CharacterBase.Dialog> loadDialogs(
      JsonObject characterObject, String language) {
        ...
        .forEach(
            key -> {
              CharacterBase.Dialog dialog =
                  loadDialog(
                      key,
                      characterObject.get("dialogs").getAsJsonObject().get(key).getAsJsonObject(),
                      language);
   ...
   * @return The dialog.
   */
  private static CharacterBase.Dialog loadDialog(
      String id, JsonObject dialogObject, String language) {
              ...
              key ->
                  options.put(
                      ...
                      loadOption(
                          id,
                          key,
                          choice.get("choices").getAsJsonObject().get(key).getAsJsonObject(),
                          language)));
   ...
   * @return The option.
   */
  private static CharacterBase.Option loadOption(
      String dialogId, String optionId, JsonObject option, String language) {
    ...
    ButtonStyle style =
        ButtonStyle.valueOf(
            option.get("name").getAsJsonObject().get("style").getAsString().toUpperCase());

    return new CharacterBase.Option(
        optionId, name, icon, style, new CharacterBase.Dialog(dialogId, answer, false, null, null));
  }
}
