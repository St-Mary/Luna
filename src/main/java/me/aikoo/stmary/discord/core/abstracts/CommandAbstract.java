package me.aikoo.stmary.discord.core.abstracts;
...
import me.aikoo.stmary.discord.core.managers.ButtonManager;
import me.aikoo.stmary.discord.core.managers.CooldownManager;
...
public abstract class CommandAbstract {
  ...
  /**
   * Get whether the command requires the user to be registered in-game. Set the command to require
   * the user to be registered in-game.
   */
  @Getter @Setter protected boolean mustBeRegistered = true;

  /**
   * Get whether the command is an administrator command. Set the command to be an administrator
   * command.
   */
  @Getter @Setter protected boolean isAdminCommand = false;
  ...
  @Getter protected Long cooldown = 5000L;
  ...
  public void run(SlashCommandInteractionEvent event) {
    ...
    if (this.cooldown > 0) {
      ...
      // If the user has a cooldown, we check if it's over. If not, we send an error message,
      // otherwise we add a new cooldown.
      if (CooldownManager.hasCooldown(event.getUser().getId(), this.name)
          && CooldownManager.getRemainingCooldown(event.getUser().getId(), this.name) > 0) {
        long timeRemaining =
            CooldownManager.getRemainingCooldown(event.getUser().getId(), this.name);
        long timestampRemaining =
            TimeUnit.MILLISECONDS.toSeconds(System.currentTimeMillis() + timeRemaining);
    ...
    // If the command requires the user to be registered in-game, we check if he is. If not, we send
    // an error message.
  ...
  public SlashCommandData buildCommandData() {
    LocalizationFunction localizationFunction =
        ResourceBundleLocalizationFunction.fromBundles("bundles/Commands", DiscordLocale.FRENCH)
            .build();
    SlashCommandData data =
        Commands.slash(this.name, this.description).setLocalizationFunction(localizationFunction);
