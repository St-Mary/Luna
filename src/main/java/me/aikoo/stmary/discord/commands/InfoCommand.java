...
import me.aikoo.stmary.discord.core.bases.*;
import me.aikoo.stmary.discord.core.bot.StMaryClient;
import me.aikoo.stmary.discord.core.managers.LocationManager;
import me.aikoo.stmary.discord.core.managers.ObjectManager;
...
public class InfoCommand extends CommandAbstract {
  ...
  public InfoCommand(StMaryClient stMaryClient) {
    ...
    Command.Choice objectChoice =
        new Command.Choice(TextManager.getText("info_choice_object", "en"), "object")
            .setNameLocalization(
                DiscordLocale.FRENCH, TextManager.getText("info_choice_object", "fr"));
    Command.Choice placeChoice =
        new Command.Choice(TextManager.getText("info_choice_place", "en"), "place")
            .setNameLocalization(
                DiscordLocale.FRENCH, TextManager.getText("info_choice_place", "fr"));
    Command.Choice characterChoice =
        new Command.Choice(TextManager.getText("info_choice_character", "en"), "character")
            .setNameLocalization(
                DiscordLocale.FRENCH, TextManager.getText("info_choice_character", "fr"));
    Command.Choice monsterChoice =
        new Command.Choice(TextManager.getText("info_choice_monster", "en"), "monster")
            .setNameLocalization(
                DiscordLocale.FRENCH, TextManager.getText("info_choice_monster", "fr"));
    Command.Choice questChoice =
        new Command.Choice(TextManager.getText("info_choice_quest", "en"), "quest")
            .setNameLocalization(
                DiscordLocale.FRENCH, TextManager.getText("info_choice_quest", "fr"));
    Command.Choice titleChoice =
        new Command.Choice(TextManager.getText("info_choice_title", "en"), "title")
            .setNameLocalization(
                DiscordLocale.FRENCH, TextManager.getText("info_choice_title", "fr"));

    this.options.add(
        new OptionData(OptionType.STRING, "element", "The element to get informations about", true)
            .addChoices(
                objectChoice,
                placeChoice,
                characterChoice,
  ...
  public void execute(SlashCommandInteractionEvent event, String language) {
    String element =
        (event.getOption("element") != null)
            ? Objects.requireNonNull(event.getOption("element")).getAsString()
            : null;
    String name =
        (event.getOption("name") != null)
  ...
  private void infoPlace(SlashCommandInteractionEvent event, String name, String language) {
    ...
    String content =
        TextManager.getText("info_place_description", language)
            .replace("{{icon}}", location.getIcon())
            .replace("{{name}}", location.getName(language))
            .replace("{{type}}", location.getType())
            .replace("{{description}}", description);

    if (location instanceof TownBase town) {
      String townPlaces = this.formatTownPlaceDescription(town, language);
      content +=
          TextManager.getText("info_place_description_town", language)
              .replace("{{town_places}}", townPlaces);
      ...
      content +=
          TextManager.getText("info_place_description_place", language)
              .replace("{{available_destinations}}", availableDestinations);
    }
  ...
  private String formatAvailableDestination(PlaceBase place, String language) {
    ...
    for (JourneyBase journey : availableDestinationsJourney) {
      availableDestinations.append(
          String.format("%s `%s`", journey.getTo().getIcon(), journey.getTo().getName(language)));
      if (availableDestinationsJourney.indexOf(journey)
          != availableDestinationsJourney.size() - 1) {
        availableDestinations.append(", ");
      }
    }
  ...
  private String formatTownPlaceDescription(TownBase town, String language) {
    ...
    for (PlaceBase place : places) {
      formattedDescription.append(
          String.format("**%s** `%s`", place.getIcon(), place.getName(language)));
  ...
  private String formatRegionPlaceDescription(RegionBase region, String language) {
    ...
    for (TownBase town : towns) {
      formattedDescription.append(
          String.format("**%s** `%s`", town.getIcon(), town.getName(language)));
  ...
  private void infoObject(SlashCommandInteractionEvent event, String name, String language) {
    ...
    String text =
        TextManager.createText("info_object_description", language)
  ...
  private String infoAllTitles(String language) {
    ...
    for (TitleBase title : titles) {
      stringBuilder.append(
          String.format(
              "%s | **%s** - `%s`",
              title.getIcon(), title.getName(language), title.getDescription(language)));
    ...
    return TextManager.createText("info_title_titles", language)
  ...
  private String infoOneTitle(String id, String language) {
    ...
    return TextManager.createText("info_title_title", language)
        .replace("icon", title.getIcon())
        .replace("name", title.getName(language))
        .replace("description", title.getDescription(language))
        .build();
  }
