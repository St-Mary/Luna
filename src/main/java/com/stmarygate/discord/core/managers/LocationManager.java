...
import com.stmarygate.discord.core.bases.JourneyBase;
import com.stmarygate.discord.core.bases.PlaceBase;
import com.stmarygate.discord.core.bases.RegionBase;
import com.stmarygate.discord.core.bases.TownBase;
...
public class LocationManager {
  ...
  public static TownBase getTownByName(String name, String language) {
    return towns.stream()
        .filter(town -> town.getName(language).equalsIgnoreCase(name))
        .findFirst()
        .orElse(null);
  }
  ...
  private static HashMap<String, RegionBase> loadRegions() {
    ...
    for (JsonObject regionObject : regions) {
      ...
      region.setDescription(
          "en", regionObject.get("description").getAsJsonObject().get("en").getAsString());
      region.setDescription(
          "fr", regionObject.get("description").getAsJsonObject().get("fr").getAsString());
        ...
        .forEach(
            townObj -> {
              ...
              if (entryPointObject == null) {
                LOGGER.error(
                    "Entry point " + townObject.get("entryPoint").getAsString() + " not found!");
                System.exit(1);
              }

              // Create the entry point object
              PlaceBase entryPoint =
                  new PlaceBase(entryPointObject.get("id").getAsString(), region);
              entryPoint.setName(
                  "en", entryPointObject.get("name").getAsJsonObject().get("en").getAsString());
              entryPoint.setName(
                  "fr", entryPointObject.get("name").getAsJsonObject().get("fr").getAsString());
              entryPoint.setDescription(
                  "en",
                  entryPointObject.get("description").getAsJsonObject().get("en").getAsString());
              entryPoint.setDescription(
                  "fr",
                  entryPointObject.get("description").getAsJsonObject().get("fr").getAsString());
              ...
              town.setDescription(
                  "en", townObject.get("description").getAsJsonObject().get("en").getAsString());
              town.setDescription(
                  "fr", townObject.get("description").getAsJsonObject().get("fr").getAsString());
    ...
    json.get("places")
        ...
        .forEach(
            place -> {
              ...
              RegionBase region =
                  (location instanceof RegionBase)
                      ? (RegionBase) location
                      : ((TownBase) location).getRegion();
              ...
              p.setDescription(
                  "en", placeObject.get("description").getAsJsonObject().get("en").getAsString());
              p.setDescription(
                  "fr", placeObject.get("description").getAsJsonObject().get("fr").getAsString());
  ...
  public static String formatLocation(String id, String language) {
    ...
    // If the location is found, return its icon followed by its name; otherwise, return "Unknown
    // location"
    return (location != null)
        ? location.getIcon() + location.getName(language)
        : "Unknown Location";
  }
