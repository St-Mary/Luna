...
public class PlaceBase extends LocationAbstract {
  ...
  @Getter private final String icon = "\uD83D\uDCCD"; // You can customize this icon.
  ...
  @Getter private final ArrayList<JourneyBase> availableMoves = new ArrayList<>();

  /** Get the town to which the place belongs. Set the town to which the place belongs. */
  @Getter @Setter private TownBase town = null;
  ...
  public JourneyBase getMove(String name) {
    return this.availableMoves.stream()
        .filter(move -> move.getTo().getId().equalsIgnoreCase(name))
        .findFirst()
        .orElse(null);
  }
